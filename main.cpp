#define BOOTSTRAPPER_TITLE "OpenWF Bootstrapper v0.7.5"
...
static void* winhttp_new_request_detour(void* a1, void* a2, void* a3, char* path, bool a5)
{
	...
	{
#if SELF_HOST_CACHE_MANIFEST
		ObfusString simple_path("/H.Cache.bin");
		memcpy(path, simple_path.data(), simple_path.size() + 1);
#endif
	}
...
until yield())EOC").str());
...

		{
			...
			ServerWebService srv([](soup::Socket& s, soup::HttpRequest&& req, soup::ServerWebService&)
			{
				...
				{
#if SELF_HOST_CACHE_MANIFEST
				case soup::joaat::compileTimeHash("/H.Cache.bin"):
					{
						uint8_t bin[] = {
