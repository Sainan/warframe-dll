...
BOOL APIENTRY DllMain(HMODULE hmod, DWORD reason, PVOID)
{
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		const bool is_20_18_0_or_above = (version_compare(std::string(build_label, 16), ObfusString("2018.04.20.02.04").str()) >= 0);
		...
		// This hook allows WorldSeed to be absent or just any value.
		{
			...
			else if (is_20_18_0_or_above)
			{
				SIG_INST("48 89 5C 24 10 48 89 74 24 18 55 57 41 56 48 8D 6C 24 B9 48 81 EC 90 00 00 00 48 8B 05 ? ? ? ? 48 33 C4 48 89 45 37 48 8B D9 84 D2"); // 2018.05.17.16.28 (20.20.0), 2018.04.20.02.04 (20.18.0)
				verify_worldstate_integrity = Module(nullptr).range.scan(sig_inst).as<void*>();
			}
			else
			{
